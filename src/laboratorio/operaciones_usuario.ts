...
export class OperableUser extends User {
    ...
    addFunko(newFunko: Funko): number {
        if(this._funkoCollection.find(funko => funko.id == newFunko.id) == undefined) {
            this._funkoCollection.push(newFunko);
            return 0;
        } else {
            return -1;
        }
    ...
    modifyFunko(newFunko: Funko): number {
        ...
        if(position != -1) {
            this._funkoCollection[position] = newFunko;
            return 0;
        } else {
            return -1;
        }
    ...
    deleteFunko(idFunko: number): number {
        ...
        if(position != -1) {
            this._funkoCollection.splice(position, 1);
            return 0;
        } else {
            return -1;
        }
    ...
    showFunkoInfo(idFunko: number): number {
        const index = this._funkoCollection.findIndex(funko => funko.id == idFunko);
        if(index != -1) {
            console.log("----------------------------------")
            console.log(`ID: ${this._funkoCollection[index].id}`);
            console.log(`Nombre: ${this._funkoCollection[index].name}`);
            console.log(`Descripcion:  + ${this._funkoCollection[index].description}`);
            console.log(`Tipo:  + ${this._funkoCollection[index].type}`);
            console.log(`Género:  + ${this._funkoCollection[index].gender}`);
            console.log(`Franquicia:  + ${this._funkoCollection[index].franchise}`);
            console.log(`Número:  + ${this._funkoCollection[index].number}`);
            console.log(`Exlusivo:  + ${this._funkoCollection[index].exclusive}`);
            console.log(`Características:  + ${this._funkoCollection[index].specialCharacteristics}`);
            if(this._funkoCollection[index].value < 20) {
                log("Valor: " + chalk.red(`${this._funkoCollection[index].value}`))
            } else if (this._funkoCollection[index].value < 50) {
                log("Valor: " + chalk.magenta(`${this._funkoCollection[index].value}`))
            } else if (this._funkoCollection[index].value < 200) {
                log("Valor: " + chalk.yellow(`${this._funkoCollection[index].value}`))
            } else {
                log("Valor: " + chalk.green(`${this._funkoCollection[index].value}`))
            }
            return 0;
        } else {
            return -1;
